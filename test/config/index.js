...
describe('Test balm config', () => {
  describe('#source', () => {
    ...
    it('source.css should return `path/to/workspace/src/styles`', () => {
      assert.equal(
        balm.config.source.css,
        path.join(appDir, balmConfig.paths.source.css)
      );
    });
    it('source.js should return `path/to/workspace/src/scripts`', () => {
      assert.equal(
        balm.config.source.js,
        path.join(appDir, balmConfig.paths.source.js)
      );
    });
    it('source.img should return `path/to/workspace/src/images`', () => {
      assert.equal(
        balm.config.source.img,
        path.join(appDir, balmConfig.paths.source.img)
      );
    });
    it('source.font should return `path/to/workspace/src/fonts`', () => {
      assert.equal(
        balm.config.source.font,
        path.join(appDir, balmConfig.paths.source.font)
      );
    });
  });
  ...
  describe('#tmp', () => {
    ...
    it('tmp.css should return `path/to/workspace/.tmp/css`', () => {
      assert.equal(
        balm.config.tmp.css,
        path.join(tmpDir, balmConfig.paths.target.css)
      );
    });
    it('tmp.js should return `path/to/workspace/.tmp/js`', () => {
      assert.equal(
        balm.config.tmp.js,
        path.join(tmpDir, balmConfig.paths.target.js)
      );
    });
    it('tmp.img should return `path/to/workspace/.tmp/img`', () => {
      assert.equal(
        balm.config.tmp.img,
        path.join(tmpDir, balmConfig.paths.target.img)
      );
    });
    it('tmp.font should return `path/to/workspace/.tmp/fonts`', () => {
      assert.equal(
        balm.config.tmp.font,
        path.join(tmpDir, balmConfig.paths.source.font)
      );
    });
  });
  ...
  describe('#target', () => {
    ...
    it('target.css should return `path/to/workspace/dist/css`', () => {
      assert.equal(
        balm.config.target.css,
        path.join(distAssetsDir, balmConfig.paths.target.css)
      );
    });
    it('target.js should return `path/to/workspace/dist/js`', () => {
      assert.equal(
        balm.config.target.js,
        path.join(distAssetsDir, balmConfig.paths.target.js)
      );
    });
    it('target.img should return `path/to/workspace/dist/img`', () => {
      assert.equal(
        balm.config.target.img,
        path.join(distAssetsDir, balmConfig.paths.target.img)
      );
    });
    it('target.font should return `path/to/workspace/dist/font`', () => {
      assert.equal(
        balm.config.target.font,
        path.join(distAssetsDir, balmConfig.paths.target.font)
      );
    });
  ...
  describe('#assets', () => {
    let assetsDir = path.join(
      balmConfig.assets.root,
      balmConfig.assets.publicPath,
      balmConfig.assets.subDir
    );
    ...
    it('assets.css should return `path/to/remote/assets/css`', () => {
      assert.equal(
        balm.config.assets.css,
        path.join(assetsDir, balmConfig.paths.target.css)
      );
    });
    it('assets.js should return `path/to/remote/assets/js`', () => {
      assert.equal(
        balm.config.assets.js,
        path.join(assetsDir, balmConfig.paths.target.js)
      );
    });
    it('assets.img should return `path/to/remote/assets/img`', () => {
      assert.equal(
        balm.config.assets.img,
        path.join(assetsDir, balmConfig.paths.target.img)
      );
    });
    it('assets.font should return `path/to/remote/assets/font`', () => {
      assert.equal(
        balm.config.assets.font,
        path.join(assetsDir, balmConfig.paths.target.font)
      );
    });
  });
});
