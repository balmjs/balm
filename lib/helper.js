...
const init = () => {
  ...
  // create assets dir
  config.assets.base = path.join(config.assets.root, config.assets.publicPath, config.assets.subDir);
  config.assets.html = path.join(config.assets.root, config.assets.publicPath);
  let assets = ['css', 'js', 'img', 'font'];
  for (let asset of assets) {
    config.assets[asset] = path.join(config.assets.root, config.assets.publicPath, config.assets.subDir, config.paths.target[asset]);
  }
...
const assetsPath = _path => {
  return path.posix.join(config.assets.subDir, _path);
};
