...
const gulpSftp = options => {
  ...
  let showLog = options.showLog || false;
  ...
  const pool = (remotePath, uploader) => {
    ...
    if (options.password) {
      logger.info('[SFTP]', 'Authenticating with password.', true);
    } else if (key) {
      logger.info('[SFTP]', 'Authenticating with private key.', true);
    }
    ...
    conn.on('ready', () => {
      conn.sftp((err, sftp) => {
        ...
        sftp.on('end', function() {
          logger.info('[SFTP]', 'SFTP session closed', true);
          sftpCache = null;
          if (!finished) {
            this.emit(
              'error',
    ...
    conn.on('end', () => {
      logger.info('[SFTP]', 'Connection :: end', true);
    });

    conn.on('close', hadError => {
      if (!finished) {
        logger.info('[SFTP]', 'SFTP abrupt closure', true);
          ...
          next => {
            ...
            sftp.mkdir(d, { mode: '0755' }, err => {
              ...
              if (err) {
                ...
                logger.success('[SFTP Created]', d, showLog);
              }
