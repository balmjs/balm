class Html extends BalmJS.Task {
  ...
  getAssetsPath(type) {
    let developmentPublicPath = new RegExp(config.html.regex[type], 'g');
    let productionPublicPath = config.html.replacement.begin + path.join(config.assets.subDir, config.paths.target[type]) + config.html.replacement.end;
