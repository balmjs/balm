...
class Sprite {
  constructor() {
    ...
    for (let key = 0, len = spriteList.length; key < len; key++) {
      ...
      let spriteConfig = {
        ...
        img: !config.static ? config.dist.img : config.app.img,
        css: config.app.css + config.sprites.cssPath // don't modify
      };
  ...
  getOption(name) {
    let spriteImgPath = (config.production || !config.static) ? config.paths.dist.img : config.paths.app.img;
