/**
 * 3. Background & Inline SVG
 */
class Svg extends Task {
  constructor() {
    super('svg');

    this.tasks = [];

    if (config.sprites.svg.length) {
      for (let svgName of config.sprites.svg) {
        let svgConfig = this.getOption(svgName);
        let svgTaskName = this.name + ':' + svgName;

        gulp.task(svgTaskName, () => {
          return gulp.src(config.source.img + '/' + svgName + '/*.svg')
            .pipe($.svgSprite(svgConfig))
            .pipe(gulp.dest(this.dist));
        });

        this.tasks.push(svgTaskName);
      }
    }
  }
  getOption(name) {
    let extensions = (config.styles.ext === 'sass') ? 'scss' : config.styles.ext;

    // generate style
    let render = {};
    render[extensions] = {
      dest: '../../' + config.roots.source + '/' + config.paths.source.css + '/svg/_' + name + '.' + extensions // relative path
    };

    // generate svg
    let mode = {};
    let basePath = (config.styles.ext === 'css') ? '/' : config.sprites.basePath;
    mode[config.sprites.mode] = {
      dest: '.',
      sprite: basePath + config.paths.target.img + '/' + name + '-' + config.sprites.mode + '.svg',
      bust: false, // use `gulp-rev`
      render: render
    };

    return {
      mode: mode
    };
  }
  get deps() {
    return this.tasks;
  }
}

export default Svg;
