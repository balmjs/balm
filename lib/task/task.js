class Task {
  constructor(name) {
    ...
    switch (name) {
      ...
      case 'publish':
        this.input = [
          path.join(config.roots.target, '**', '*'),
          '!' + path.join(config.roots.target, '*.*')
        ];
        this.output = path.join(config.publish.root, config.publish.publicPath, config.publish.subDir);
        this.publicCssPath = config.publish.replacement.begin + path.join(config.publish.subDir, config.paths.target.css) + config.publish.replacement.end;
        this.publicJsPath = config.publish.replacement.begin + path.join(config.publish.subDir, config.paths.target.js) + config.publish.replacement.end;
        break;
      default:
    }
  }
