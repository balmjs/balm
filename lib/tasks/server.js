...
class Server extends Task {
  ...
  watch() {
    ...
    gulp.watch(`${config.source.css}/**/*.${config.styles.ext}`, getNamespace([this.getStyleTask()]));
    ...
    gulp.watch(`${config.workspace}/bower.json`, getNamespace(['wiredep', 'fonts']));
  }
        ...
        files: [
          `${config.source.js}/**/*`,
          {
            ...
            fn: (event, file) => {
              ...
              if (event === 'add') {
                gulp.watch(File.absPaths(file), getNamespace([this.getStyleTask()]));
              }
            }
          }
        ],
