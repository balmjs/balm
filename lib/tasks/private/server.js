...
class ServerTask extends BalmStyleTask {
  ...
  onWatch() {
    ...
    if (config.scripts.entry && !config.scripts.hot) {
      watch(`${config.source.js}/**/*`, watchOptions, series(toNamespace('scripts'), reload));
    }

    watch(`${config.source.base}/*.html`, watchOptions, parallel(toNamespace('html'))).on(
      'change',
      server.reload
    );
    watch(`${config.source.font}/**/*`, watchOptions, parallel(toNamespace('fonts')));
    watch(
