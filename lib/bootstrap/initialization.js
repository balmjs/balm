...
const init = () => {
  ...
  // Sync source path name for html
  for (let value of ['css', 'js', 'img']) {
    config.html.regex[value] = config.paths.source[value];
  }

  // Set default entry js
  if (!Object.keys(config.scripts.entry).length) {
    config.scripts.entry.main = `./${config.roots.source}/${config.paths.source.js}/main`;
  }
