...
const registerTasks = () => {
  ...
  balmTasks.forEach(gulpTask => {
    ...
    let balmTask;
    let mainTasks = [];
    switch (gulpTask.name) {
      case 'sprites':
        balmTask = series(toNamespace(gulpTask.deps));
        break;
      case 'default':
        mainTasks = toNamespace(gulpTask.deps);

        if (isString(BalmJS.beforeTask)) {
          mainTasks.unshift(BalmJS.beforeTask);
        }
        if (isString(BalmJS.afterTask)) {
          mainTasks.push(BalmJS.afterTask);
        }

        balmTask = series(mainTasks);
        break;
      default:
        balmTask = gulpTask.fn;
        break;
    }
