...
const webpackConfig = (input = '', output = '') => {
  ...
  let jsFilename = config.production ? config.scripts.filename + '.[hash:7].js' : config.scripts.filename + '.js';
  let jsChunkFilename = config.production ? config.scripts.chunkFilename + '.[chunkhash].js' : config.scripts.chunkFilename + '.js';
  jsFilename = config.scripts.filename + '.js';
  jsChunkFilename = config.scripts.chunkFilename + '.js';

  let configuration = {
    context: config.workspace,
    entry: webpackEntries,
    output: {
      path: output || config.tmp.js,
    ...
    plugins: [
      ...
      ...(config.scripts.vendors.length ? [new webpack.optimize.CommonsChunkPlugin({
        names: config.scripts.vendors.map(vendor => path.join('vendor', vendor)),
        filename: jsFilename,
        minChunks: Infinity
      })] : []),
