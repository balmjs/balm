import './global';
import {isString, isArray, mergeDeep} from './helper';
...
class Balm {
  ...
  set config(data) {
    ...
    if (data.paths && data.paths.target) {
      if (isString(data.paths.target.css)) {
        defaults.html.regex.css = data.paths.target.css;
      }
      if (isString(data.paths.target.js)) {
        defaults.html.regex.js = data.paths.target.js;
      }
    }
